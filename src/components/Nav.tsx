...
import { navDataTypes } from '@/accessories/types';
        ...
        {navData.map(({ id, name, icon, path }) => {
          return (
            <Link className={`${path === pathname && 'text-accent'} relative flex items-center  group-hover:text-accent transition-all duration-300`} href={path} key={id}>
              ...
              {/* tooltip */}
              <div className='absolute pr-14 right-0 hidden xl:group-hover:flex  hover:flex'>
                <div className='bg-white border-solid border-[1px] relative flex text-black items-center p-[6px] rounded-[3px]'>
